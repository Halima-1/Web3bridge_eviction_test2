...
## Overview
This project is designed to securely manage protocol assets, allowing governance-controlled proposals while preventing attacks commonly seen in DeFi treasury protocols ARES Protocol, a ARES manages $500M+ in treasury assets and distributes capital to contributors, liquidity providers, and governance participants.
...
### Security Boundaries
- Governors can propose, queue, execute, cancel, and update Merkle roots.
- Only valid signatures from governors allow execution.
- Timelock ensures no immediate execution.
- Claimed mapping prevents double reward withdrawals.

### Trust Assumptions
- Governors are trusted for proposal approvals.
- Users cannot bypass timelock or replay proposals.
- Treasury funds are protected by `treasuryLimit` and cryptographic checks.